...
<!-- 		<groundingDetailComponent v-if="togglegrounding" @closeDetail="togglegrounding=false" :tmid="chosePack.choseTmid"
		...
		<view class="grounding-header">
			<navbarComponent @stepClick="onStepClick" :buttonList="['查询','登记']"></navbarComponent>
		...
		<registerComponent v-if="activeTabIndex==1"></registerComponent>

         
		<view class="provide-content" @click="closeAllselect" v-if="activeTabIndex==0">
			...
			<view class="provide-list">
				<view v-for="(item,index) in lossregisterList" :key="index">
					...
					<lossregisterItemComponent   :item="item"></lossregisterItemComponent>
				</view>
				<loadingMoreComponent v-if="lossregisterList.length" :loadingType="loadingType"></loadingMoreComponent>
			</view>
		</view>
...
<script>
	...
	import lossregisterItemComponent from "../../components/lossregister/lossregister-list-item.vue";
	import registerComponent from "../../components/lossregister/register.vue";
	...
	import {
       getdeptbyfloor,getfloors, getbuilds
	...
	export default {
		...
		data() {
			return {
				...
				timestate: '',
				lossregisterList:[1,2,3,4,5,5,5,5,5,5,5,2,5,,5,3,55,,5,5],
				showdetail:false,
				tmid:'',
				loadingType: 2,
				buildSelect:false,
...
// 				getlossregisterList(data).then(res=>{
// // 					console.log(res,'申领单列表');
// // 					if(res.status=="OK"){
// // 						this.lossregisterList=res.returnValue.FfList;
// // 						this.subtime(this.lossregisterList);
// // 					}else{
