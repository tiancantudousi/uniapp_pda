		...
		<view slot="content">
			<view class="washing-content">
				...
				<scroll-view scroll-y="true" class="home-content" :style="{'height':homeHeight+'px'}" @scrolltolower="scrolltoBottom">
					<view v-for="(item,index) in detailsterilization" :key="index">
				...
				<view class="start-wash flexaround">
				     <view @click.stop="update">修改</view>
					  <view @click.stop="showDialog=true">作废</view>
				</view>
			<!-- 	<view class="washing-main-noplate">
				...
				<selfDialogComponent v-if="showDialog">
					...
					<view slot="footer">
						...
						<button type="primary" size="mini"  @click.stop="canclesterilization">是</button>
					</view>
...
<script>
	...
	export default {
		...
		computed: {
			...mapGetters(["loginForm","activesterilizationMachine","detailsterilization"])
		},
		created() {
			 this.activeMachine=washingitem.sterilizationitem;
			 console.log(this.activeMachine,'this.activeMachine');	
		...
		methods: {
              canclesterilization(){
				  console.log(this.activesterilizationMachine,"activesterilizationMachine");
				  let Mjdata={...this.activesterilizationMachine,"is_inv":"1"}
				  const data={"Mj":Mjdata,"LoginForm":this.loginForm};
				  canclesterilization(data).then(res=>{
					  console.log(res,"灭菌作废");
					  	if(res.errorCode=="0"){
							this.toast("作废成功");
			...
			update(){
				 uni.navigateTo({url:'/pages/sterilizationfree/sterilizationfree?id=update'});
			},
			onSelected(e){
